...
public class AddNewBook extends AbstractTest{
    ...
    void AddBook(){
        given()
                ...
                .body("book.id", Matchers.is(Matchers.notNullValue())) //ПРОВЕРИТЬ И ПЕРЕДЕЛАТЬ
    ...
    void AddBookYearOfFiveCharacters(){
        given()
                ...
                //Сделать проверку что книга не может быть написана раньше текущей даты
    }
    @Test
    void AddBookInvalidDate(){
        given()
                ...
                .statusLine(containsString("BAD REQUEST"))
                .body("error", Matchers.is("Invalid date format"));
        //Сделать проверку не верный формат даты
    }
